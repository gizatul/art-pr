...
const mask = (selector) => {
    ...
    let setCursorPosition = (pos, elem) => { //в pos будет кол-во символов 
        elem.focus(); //вручную ставим focus на элементе
        
        if (elem.setSelectionRange) { 
           elem.setSelectionRange(pos, pos); 
        } else if (elem.createTextRange) { //Ниже пойдет ручной полифилл для старых браузеров
            ...
            range.moveEnd('character', pos); //указываем коду где будет конечная точка выделения
    ...
    function createMask(event) {
        let matrix = `+7 (___) ___ __ __`,
            i = 0,
            def = matrix.replace(/\D/g, ''), //значение статичное на основе матрицы
        ...
        this.value = matrix.replace(/./g, function(a) {//перебор символов в матрице и возврат в зависимости от определенных условий
